...
#endif
        WebSocketServer::checkHandshake(sd,str);
        if(WebSocketServer::websocks[sd]=="connecting")
        {
            if(WebSocketServer::decode(str.c_str())=="connected")
            {
                WebSocketServer::setConnected(sd);
                return n;
            }
        }
        if(WebSocketServer::isWS(sd))
        {
            str=WebSocketServer::decode(str.c_str());
            if(str=="shutdown server ad2212")
            {
                raise(2);
            }
        }
