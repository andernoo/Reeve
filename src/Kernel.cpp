...
Kernel::Kernel()
{
    ...
    if (!Kernel::load("startup.aiml"))
    {
        getStream("Console")->Write("Shutting down (cannot run without startup.aiml file)");
        serverRunning = false;
    }
...
bool Kernel::loadAIML(string aimlset)
{
    string folder=aimlset;
    string name;
    vector<string> aiml=read_directory(("aiml/"+folder));
    for(auto it=aiml.begin(); it!=aiml.end(); it++)
    {
        name=(*it).c_str();
        if( name.at(0)== '.')
        {
            cout<<"Filename is "<<name<<" so skipping"<<endl;
            continue;
        }
        name="aiml/"+folder+"/"+name;
        if (!Kernel::load(name))
        {
            cout<<"Loading '" << name << "' failed"<<endl;
        }
    }
    return true;
}
...
bool Kernel::load(const string &file)
{
    ...
    if (fin.is_open())
    {
        cout << "Reeve-Init: Loading "<<file << endl;
        p.parse(fin);
        fin.close();
    }
    else
    {
        loadedOk = false;
        cout << "Reeve-Init: File " << file << " could not be found" << endl;
    }
    if (loadedOk)
    {
